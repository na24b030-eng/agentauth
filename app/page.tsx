...
export default function Home() {
  ...
  async function resetDemo() {
    ...
    setBusy(true); setError('');
    try {
      await requestJson(merchantBase(), '/v1/developer/reset-demo', token, { method: 'POST' });
      runGeneration.current += 1;
      setGrant(null); setRun(null); setToolEvents([]); setQuote(null); setCheckout(null); setAudit([]);
      setSubmittedMessage(''); setDraft('Order my usual groceries under ₹900 for delivery tonight');
      setArmedFaults([]); setView('delegations');
      setNotice('Local fictional state reset. Approve a fresh bounded grant to continue.');
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not reset demo state'); }
    finally { setBusy(false); }
  }

  if (connection === 'checking') return <main className="boot-screen"><div className="brand-mark"><ShieldCheck size={18} /></div><p>Connecting to authorization services…</p></main>;
  if (connection === 'login') return <main className="login-screen"><div className="login-shell"><div className="login-context"><span className="brand-mark"><ShieldCheck size={18} /></span><div><b>AgentAuth</b><span>Delegated commerce control plane</span></div></div><div><div className="section-kicker">SECURE TEST WORKSPACE</div><h1>Sign in to review delegated purchases.</h1><p>AgentAuth separates an AI buyer’s proposal from the authority and payment controls that decide whether it may execute.</p><ul><li><ShieldCheck size={15} /> Agent-bound proof of possession</li><li><CircleDollarSign size={15} /> Hard spending and category limits</li><li><ReceiptText size={15} /> Durable checkout and audit state</li></ul></div></div><form className="login-card" onSubmit={login}><div className="login-card-head"><LockKeyhole size={19} /><div><b>Demo identity</b><span>Application consent only</span></div></div><label>Email<input value={email} onChange={(e) => setEmail(e.target.value)} type="email" /></label><label>Passcode<input value={passcode} onChange={(e) => setPasscode(e.target.value)} type="password" /></label>{error && <div className="error-banner">{error}</div>}<button disabled={busy}>{busy ? 'Verifying…' : 'Enter test environment'}<ArrowRight size={15} /></button><p>This is not bank, biometric, or device identity.</p></form></main>;
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="topbar-inner"><div className="brand-lockup"><span className="brand-mark"><ShieldCheck size={18} /></span><div><strong>AgentAuth</strong><span>Delegated commerce control plane</span></div></div><div className="topbar-meta"><div className={`environment ${connection === 'preview' ? 'preview' : ''}`}><i />{connection === 'preview' ? 'Preview fixture' : 'Test services online'}</div><button className="account" aria-label="Current demo user"><span>{displayName.slice(0, 2).toUpperCase()}</span><div><b>{displayName}</b><small>Demo account</small></div></button></div></div></header>
    <nav className="product-nav" aria-label="Primary navigation"><div>{navItems.map(([key, Icon, label]) => <button key={key} className={view === key ? 'active' : ''} onClick={() => setView(key)}><Icon size={15} />{label}</button>)}</div></nav>
    {(error || notice) && <div className={error ? 'global-banner error' : 'global-banner'}><span>{error ? <AlertTriangle size={16} /> : <BadgeCheck size={16} />}{error || notice}</span><button aria-label="Dismiss notification" onClick={() => { setError(''); setNotice(''); }}><X size={15} /></button></div>}
    <div className="workspace">
      <section className="conversation">
        {view === 'commerce' ? <>
          <div className="page-heading"><div><div className="section-kicker">{connection === 'preview' ? 'GUIDED PRODUCT PREVIEW' : 'ACTIVE COMMERCE SESSION'}</div><h1>Purchase request <small>Good evening, {displayName}.</small></h1><p>Describe the outcome. The agent can prepare a basket; deterministic controls decide whether it can proceed.</p></div><div className="mode-control"><span>Execution mode</span><div className="mode-switch" aria-label="Payment mode"><button type="button" className={mode === 'DELEGATED_DEBIT_SIMULATOR' ? 'selected' : ''} onClick={() => setMode('DELEGATED_DEBIT_SIMULATOR')}>Simulator</button><button type="button" className={mode === 'RAZORPAY_PAYMENT_LAB' ? 'selected' : ''} onClick={() => setMode('RAZORPAY_PAYMENT_LAB')} disabled={!razorpayConfigured} title={razorpayConfigured ? 'Create a real Razorpay Test Mode Order' : 'Add Razorpay Test Mode credentials to enable this lab'}>Razorpay Test</button></div><small className="mode-help">{mode === 'DELEGATED_DEBIT_SIMULATOR' ? (razorpayConfigured ? 'No real payment. Same authorization pipeline.' : 'Simulation only · Razorpay Test Mode is not configured.') : 'Creates a Test Mode Order; paid only after capture.'}</small></div></div>
          <div className="session-card">
            <form className="composer" onSubmit={startRun}><label htmlFor="purchase-request">Instruction to authorized agent</label><textarea id="purchase-request" aria-label="Message your commerce agent" placeholder="For example: Order my usual groceries under ₹900 for delivery tonight" value={draft} onChange={(event) => setDraft(event.target.value)} disabled={busy} /><div className="composer-bar"><span title="The model can read history, search catalog, check delivery and request a quote. Checkout appears only after a valid quote and matching grant."><LockKeyhole size={13} /> Catalog, history, delivery and quote only until authorization passes</span><button type="submit" aria-label="Send message" disabled={busy}>{busy ? <Activity size={16} /> : <><span>Run request</span><Send size={15} /></>}</button></div></form>
            {!submittedMessage && !busy && !run && <div className="session-empty"><div><ShieldCheck size={19} /></div><section><b>Ready for a bounded purchase request</b><p>{connection === 'preview' ? 'This is a labelled preview fixture. Connect the merchant and agent APIs to execute a run.' : 'No checkout can execute without a current grant, canonical quote, allowance reservation and inventory reservation.'}</p></section><span>4 deterministic gates</span></div>}
            <div className="chat-stream">
              {submittedMessage && <div className="request-entry"><div><UserRound size={15} /><span>Your instruction</span></div><p className="user-message">{submittedMessage}</p></div>}
              {(submittedMessage || busy || run) && <div className="agent-response"><div className="response-heading"><div className="agent-orb"><Search size={15} /></div><div><span>Commerce agent</span><small>{busy ? 'Working with merchant facts' : 'Proposal complete'}</small></div></div><div className="response-copy"><p>{run?.final_response || (busy ? 'Reading merchant facts. Authority and reservations will be evaluated independently.' : connection === 'preview' ? 'Preview: the usual basket fits the sample grant and delivery constraint.' : 'Ready.')}</p><div className="live-tools" aria-live="polite">{traceSteps.map((step, index) => <span key={`${step.label}-${index}`} className={step.done ? 'complete' : 'running'}>{step.done ? <Check size={12} /> : <Activity size={12} />}{step.label}</span>)}</div>
                {quote && <div className="basket-card"><div className="basket-head"><div><span>CANONICAL QUOTE {connection === 'preview' && '· PREVIEW'}</span><h2>Merchant-calculated basket</h2></div><div className="quote-total"><strong>{money(quote.total_paise)}</strong><small>Includes {money(quote.delivery_fee_paise)} delivery</small></div></div><div className="basket-items">{quote.items.map((item) => <div className="basket-row" key={item.sku}><div><b>{item.product_name}</b><span>{item.quantity} × {money(item.unit_price_paise)}</span></div><strong>{money(item.line_total_paise)}</strong></div>)}</div><div className="basket-footer"><span><Clock3 size={13} />{quoteSeconds ? `Valid for ${Math.floor(quoteSeconds / 60)}:${String(quoteSeconds % 60).padStart(2, '0')}` : quote.status}</span><button onClick={() => setView('inspector')}>Inspect authorization<ChevronRight size={14} /></button></div></div>}
                {checkout && <div className={`checkout-card ${checkout.test_fixture_applied ? 'fixture' : ''}`}><div className="checkout-icon"><ReceiptText size={17} /></div><div><span>CHECKOUT STATE {checkout.test_fixture_applied && '· DEVELOPER FIXTURE'}</span><strong>{checkout.status.replaceAll('_', ' ')}</strong><small>{checkout.receipt} · version {checkout.version}{checkout.test_fixture_applied && ' · not a provider payment'}</small></div>{checkout.status === 'CANCEL_WINDOW' && <button onClick={cancelCurrent}>Cancel before execution</button>}{mode === 'RAZORPAY_PAYMENT_LAB' && checkout.status === 'PAYMENT_PENDING' && <button onClick={openRazorpay}>Open Razorpay Test Checkout</button>}</div>}
              </div></div>}
            </div>
          </div>
        </> : view === 'delegations' ? <div className="feature-view"><div className="page-heading compact"><div><div className="section-kicker">USER-ISSUED AUTHORITY</div><h1>Delegation consent</h1><p>Review the exact purchasing authority bound to this registered agent key.</p></div></div><div className="consent-card"><div className="consent-agent"><span><KeyRound size={18} /></span><div><b>{agent?.name || 'AgentAuth Commerce Agent v1'}</b><code>SHA256 {shortFingerprint(agent?.jwk_thumbprint)}</code></div><i>{grant?.status || 'NOT APPROVED'}</i></div><dl><div><dt>Merchant</dt><dd>AgentAuth Daily</dd></div><div><dt>Categories</dt><dd>{grant?.allowed_categories.join(', ') || 'Bakery, breakfast, dairy, produce, staples'}</dd></div><div><dt>Per order</dt><dd>{money(grant?.per_order_limit_paise || 100000)}</dd></div><div><dt>Cumulative</dt><dd>{money(grant?.cumulative_limit_paise || 300000)}</dd></div><div><dt>Expires</dt><dd>{grant ? new Date(grant.expires_at).toLocaleDateString('en-IN') : '7 days after approval'}</dd></div><div><dt>Auto-execute</dt><dd>{grant?.auto_execute === false ? 'Disabled' : 'Enabled'}</dd></div></dl><div className="demo-disclosure"><ShieldCheck size={16} /><div><b>Application-level demo identity</b><p>This consent is not bank, biometric, or device identity.</p></div></div>{connection === 'preview' ? <button disabled>Connect backend to approve</button> : grant?.status === 'ACTIVE' ? <button className="danger-button" disabled={busy} onClick={revokeGrant}>Revoke for new purchases</button> : <button disabled={busy} onClick={createGrant}>{busy ? 'Approving…' : 'Approve bounded authority'}<ArrowRight size={15} /></button>}</div></div>
        : view === 'developer' ? <div className="feature-view"><div className="page-heading compact"><div><div className="section-kicker">CONTROLLED FAILURE TESTING</div><h1>Recovery lab</h1><p>Reproduce the incidents that usually appear only under retries, timeouts and reordered events.</p></div></div><div className="developer-grid"><button disabled={connection !== 'ready'} onClick={() => armFault('DROP_ORDER_CREATE_RESPONSE')}><span className="lab-icon"><Activity size={17} /></span><div><span>Lost create response</span><b>{armedFaults.includes('DROP_ORDER_CREATE_RESPONSE') ? 'ARMED ONCE' : 'ARM WORKER'}</b><small>Discard success, then recover exactly one Order by receipt</small></div><ChevronRight size={16} /></button><button disabled={connection !== 'ready' || !grant} onClick={replayNonce}><span className="lab-icon"><RotateCcw size={17} /></span><div><span>Replay PoP nonce</span><b>RUN SIGNED REPLAY</b><small>First request succeeds; identical proof returns PROOF_REPLAYED</small></div><ChevronRight size={16} /></button><button disabled={connection !== 'ready' || !checkout?.razorpay_order_id || checkout.payment_mode !== 'RAZORPAY_PAYMENT_LAB'} onClick={runWebhookFixture}><span className="lab-icon"><ReceiptText size={17} /></span><div><span>Out-of-order webhook</span><b>RUN DISCLOSED FIXTURE</b><small>Capture, stale failure and duplicate delivery produce one ledger effect</small></div><ChevronRight size={16} /></button><button disabled={connection !== 'ready'} onClick={() => armFault('FORCE_MODEL_TIMEOUT')}><span className="lab-icon"><Clock3 size={17} /></span><div><span>Model timeout</span><b>{armedFaults.includes('FORCE_MODEL_TIMEOUT') ? 'ARMED ONCE' : 'ARM AGENT'}</b><small>The next run returns a typed recoverable outcome</small></div><ChevronRight size={16} /></button><button disabled={connection !== 'ready' || busy} onClick={resetDemo}><span className="lab-icon"><RotateCcw size={17} /></span><div><span>Reset local demo</span><b>RESET FICTIONAL STATE</b><small>Available only when Razorpay credentials are absent</small></div><ChevronRight size={16} /></button></div><div className={`config-health ${connection === 'preview' ? 'warning' : ''}`}><i />{connection === 'preview' ? 'Frontend preview only — no backend or provider claim is being made.' : 'Agent and merchant APIs are reachable. Provider secrets remain server-side.'}</div></div>
        : <div className="feature-view inspector-focus"><div className="page-heading compact"><div><div className="section-kicker">TAMPER-EVIDENT EVIDENCE</div><h1>Trust Inspector</h1><p>Follow the signed request from agent proof through every money-relevant state transition.</p></div></div><div className="canonical-block"><div><Code2 size={17} /><span>SIGNED CANONICAL REQUEST</span></div><code>TC-POP-V1{`\n`}POST{`\n`}/v1/checkouts{`\n`}raw_body_sha256 · timestamp · nonce{`\n`}{grant?.immutable_digest || 'grant_immutable_digest'}</code></div><div className="hash-chain">{(audit.length ? audit : [{ sequence: 1, action: 'quote.preview' }, { sequence: 2, action: 'checkout.reserved' }]).map((item, index) => <div key={`${item.sequence}-${item.action}`}><span>{String(item.sequence).padStart(2, '0')} {item.action}</span>{index < (audit.length ? audit.length : 2) - 1 && <i />}</div>)}</div>{audit.length > 0 && <div className="audit-list">{audit.map((event) => <div key={event.id}><span>{String(event.sequence).padStart(2, '0')}</span><section><b>{event.reason_code}</b><p>{event.explanation}</p><code>{event.event_hash.slice(0, 18)}…</code></section></div>)}</div>}</div>}
      </section>
      <aside className="trust-panel"><div className="trust-head"><div><span className="pulse"><ShieldCheck size={17} /></span><div><h2>Execution authority</h2><p>Effective controls for this session</p></div></div><span className={`status-badge ${grant?.status === 'ACTIVE' ? 'active' : ''}`}>{grant?.status || (connection === 'preview' ? 'PREVIEW' : 'MISSING')}</span></div><div className="trust-section"><div className="trust-label">REGISTERED AGENT <span>{agent ? 'KEY VERIFIED' : 'DEMO KEY'}</span></div><div className="fingerprint"><div className="fingerprint-icon"><KeyRound size={16} /></div><div><b>{agent?.name || 'AgentAuth Buyer v1'}</b><code>{shortFingerprint(agent?.jwk_thumbprint)}</code></div></div><div className="allowance"><div><span>Grant usage</span><strong>{money(allowanceUsed)} <small>/ {money(allowanceTotal)}</small></strong></div><div className="allowance-track"><i style={{ width: `${allowancePercent}%` }} /></div><p>{money(Math.max(0, allowanceTotal - allowanceUsed))} remains available</p></div></div><div className="trust-section"><div className="trust-label">POLICY CHECKS <span className="trace-id">{run ? `RUN ${run.id.slice(0, 8)}` : 'WAITING'}</span></div><div className="decision-list">{traceSteps.length ? traceSteps.map((step, index) => <div className="decision" key={`${step.label}-${index}`}><span className={step.done ? 'check' : ''}>{step.done ? <Check size={12} /> : <Activity size={12} />}</span><div><b>{step.label}</b><small>{step.detail}</small></div></div>) : <><div className="decision"><span className="check"><Check size={12} /></span><div><b>Agent key</b><small>Proof required on protected requests</small></div></div><div className="decision"><span className="check"><Check size={12} /></span><div><b>Grant scope</b><small>Category and cumulative caps enforced</small></div></div><div className="decision"><span><Clock3 size={12} /></span><div><b>Quote and reservation</b><small>Evaluated when a request is submitted</small></div></div></>}</div></div><div className="policy-note"><div><LockKeyhole size={15} /><span>Model boundary</span></div><p>Gemini may select SKUs and quantities. The merchant service owns prices, identity, inventory, authority and payment state.</p></div><button className="audit-button" onClick={() => setView('inspector')}>Open audit evidence<ArrowRight size={14} /></button></aside>
    </div>
  </main>;
}
