...
type Checkout = { id: string; status: string; payment_mode: Mode; amount_paise: number; currency: string; receipt: string; execute_after: string; payment_deadline_at: string; version: number; razorpay_order_id: string | null };
  ...
  return <main className="app-shell">
    ...
    <div className="workspace">
      ...
      <section className="conversation">
        {view === 'commerce' ? <>
          ...
          <div className="chat-stream">
            ...
            {(submittedMessage || busy || run) && <div className="agent-response"><div className="agent-orb">AI</div><div className="response-copy"><p>{run?.final_response || (busy ? 'I’m using merchant facts, then deterministic services will independently enforce authority and reservations.' : connection === 'preview' ? 'Preview: the usual basket fits the sample grant and delivery constraint.' : 'Ready.')}</p>
              ...
              {checkout && <div className="checkout-card"><div><span>CHECKOUT STATE</span><strong>{checkout.status.replaceAll('_', ' ')}</strong><small>{checkout.receipt} · version {checkout.version}</small></div>{checkout.status === 'CANCEL_WINDOW' && <button onClick={cancelCurrent}>Cancel before execution</button>}{mode === 'RAZORPAY_PAYMENT_LAB' && checkout.status === 'PAYMENT_PENDING' && <button onClick={openRazorpay}>Open Razorpay Test Checkout</button>}</div>}
            </div></div>}
          </div>
        ...
        : view === 'developer' ? <div className="feature-view"><div className="section-kicker">FAILURE LAB</div><h1>Break it safely.</h1><p>These are evidence-backed integration scenarios, not decorative UI toggles.</p><div className="developer-grid"><button disabled><span>Lost create response</span><b>WORKER FIXTURE</b><small>Set server fault flag, then recover one Order by receipt</small></button><button disabled><span>Replay PoP nonce</span><b>INTEGRATION TEST</b><small>Persistent nonce must return PROOF_REPLAYED</small></button><button disabled><span>Duplicate webhook</span><b>INTEGRATION TEST</b><small>Inbox deduplication permits one ledger effect</small></button><button disabled><span>Force model timeout</span><b>EVAL FIXTURE</b><small>Runner returns a typed failed outcome</small></button></div><div className={`config-health ${connection === 'preview' ? 'warning' : ''}`}><i />{connection === 'preview' ? 'Frontend preview only — no backend or provider claim is being made.' : 'Agent and merchant APIs are reachable. Provider secrets remain server-side.'}</div></div>
