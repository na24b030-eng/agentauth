...
|---|---|---|---|
| Natural-language goal and substitutions | One GPT-5.5 Agents SDK agent | The problem is semantic and benefits from tool selection, comparison and clarification. Representative evals decide low vs medium reasoning. | Rules-only NLU would be brittle; a multi-agent graph adds latency and failure surfaces without independent roles. |
